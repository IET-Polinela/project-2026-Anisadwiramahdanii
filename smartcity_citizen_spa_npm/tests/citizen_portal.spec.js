...
// NOTE: Pastikan backend tersedia di host ini saat menjalankan Playwright.
//
...
//   Opsi 1 (Live Server): 'http://127.0.0.1:5500/smartcity_citizen_spa_npm/index.html'
//   Opsi 2 (http-server):  'http://localhost:8080/index.html'
//   Opsi 3 (file://):      'file:///C:/Users/.../smartcity_citizen_spa_npm/index.html'
const SPA_URL = 'http://127.0.0.1:8080/index.html';
...
/**
 * mockSPAApiUrl - Memastikan SEMUA request API di SPA mengarah ke server backend yang sama dengan BASE_URL.
 *
 * (misal: http://103.151.63.85:8005/api, http://192.168.1.5/api, dll)
 * Jika SPA memanggil endpoint API lama atau url berbeda,
 * maka route interceptor ini akan membelokkannya ke BASE_URL.
 *
 * @param {import('@playwright/test').Page} page - Objek halaman Playwright
 */
async function mockSPAApiUrl(page) {
    ...
    // Gunakan pola route untuk endpoint API
    await page.route('**/api/**', async (route) => {
        const originalUrl = route.request().url();

        if (originalUrl.startsWith(backendUrl)) {
        ...
        const newUrl = `${backendUrl}${urlObj.pathname}${urlObj.search}`;
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-01: Chart.js di Dashboard Admin ter-render dengan benar', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        // Dashboard menampilkan body tabel laporan terbaru dengan id #latestReportedBody
        // dan #latestResolvedBody. Ini adalah struktur halaman yang aktual.
        await expect(page.locator('#latestReportedBody')).toBeVisible();
        await expect(page.locator('#latestResolvedBody')).toBeVisible();
